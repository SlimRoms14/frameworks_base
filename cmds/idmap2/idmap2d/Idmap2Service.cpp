...
constexpr const char* kSlimFrameworkPath = "/system/framework/org.slim.framework-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  if (target_path == kSlimFrameworkPath) {
    if (slim_apk_cache_ == nullptr) {
      // Initialize the slim APK cache.
      auto target = TargetResourceContainer::FromPath(target_path);
      if (!target) {
        return target.GetError();
      }
      slim_apk_cache_ = std::move(*target);
    }
    return {slim_apk_cache_.get()};
  }
